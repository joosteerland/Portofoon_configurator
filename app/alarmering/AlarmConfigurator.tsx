...
const adviceCopy = {
  protect: { name: "Firecom Protect", label: "Alarmering via LTE", copy: "Apps en dedicated devices alarmeren gebruikers via mobiele netwerken en het Firecom-platform." },
  rf: { name: "RF-oproepsysteem", label: "Eigen lokale infrastructuur", copy: "Eigen zenders en alarmontvangers blijven lokaal werken zonder publiek internet of mobiele telefonie." },
  hybrid: { name: "Hybride alarmering", label: "RF met mobiele aanvulling", copy: "Alarmontvangers vormen de primaire alarmering; apps en dedicated devices bereiken aanvullende gebruikers en locaties." },
};
...
export default function AlarmConfigurator() {
  ...
  const alarmReasons = useMemo(() => {
    const reasons: string[] = [`Betrouwbaarheidsniveau ${reliability} van 4 is gekozen.`];
    if (independentOfTelecom) reasons.push("Werking zonder mobiel of internet vraagt om eigen RF-infrastructuur.");
    else reasons.push("Publieke mobiele netwerken zijn voor deze configuratie toegestaan.");
    if (locations > 1) reasons.push(`${locations} locaties vragen om minimaal ${calculation.transmitterCount} zenders.`);
    if (pagers > 0) reasons.push(`${pagers} alarmontvangers zijn opgenomen voor lokale alarmering.`);
    else if (appUsers + devices === 0) reasons.push("Kies apps, dedicated devices of alarmontvangers om ontvangers toe te voegen.");
    if (espa) reasons.push("De ESPA-koppeling zet meldingen uit de brandmeldcentrale door.");
    return reasons.slice(0, 4);
  }, [reliability, independentOfTelecom, locations, calculation.transmitterCount, pagers, appUsers, devices, espa]);
  ...
  const requestQuote = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const configuration = [
        ...
        `RF-systeem: ${calculation.includesRf ? (calculation.extendedRf ? "Uitgebreid" : "Eenvoudig") : "Niet geselecteerd"}`, `Zenders: ${calculation.transmitterCount} · inclusief buitenantenne per zender`, `ESPA/BMC: ${espa ? `Ja${bmcType ? ` · ${bmcType}` : ""}` : "Nee"}`, `Vergunning: ${permit}`, `Noodstroom: ${backup === "none" ? "Niet gekozen" : `${backup} uur`}`, `Dekkingsonderzoek: ${coverageStudy ? "Ja" : "Nee"}`,
  ...
  return (
    <main className="alarm-configurator">
      ...
      <section className="hero alarm-hero" aria-labelledby="alarm-title">
        <div className="eyebrow"><span /> Firecom alarmering</div>
        <h1 id="alarm-title">Alarmerings-<br /><em>configurator</em></h1>
        <p>Geef aan waar en hoe u wilt alarmeren. U ziet welke apparatuur nodig is en welke eenmalige en jaarlijkse kosten bekend zijn.</p>
        <div className="trust-row"><span>Prijzen excl. btw</span><span>LTE, RF of hybride</span><span>Controle door een specialist</span></div>
      </section>
      ...
      <section className="configurator-shell" aria-label="Alarmeringsconfigurator">
        <div className="questions-panel" id="alarm-vragen">
          ...
          {calculation.includesRf && <Question number="06" title="Lokale RF-infrastructuur" subtitle="De configurator schakelt automatisch naar uitgebreide centrale apparatuur wanneer dat technisch nodig is.">
            ...
            <div className="inline-alert transmitter-guidance"><b>Richtlijn voor het aantal zenders</b><span>Een kleine fabriek of een kantoorpand heeft meestal voldoende aan 1 zender.</span><span>Een grote fabriek gebruikt doorgaans 2 zenders op strategische daklocaties.</span><span>Iedere zender is inclusief buitenantenne. De definitieve plaatsing volgt uit de technische controle.</span></div>
        ...
        <aside className="result-panel alarm-result" aria-live="polite">
          ...
          <div className="recommendation-reasons"><b>Waarom deze oplossing</b><ul>{alarmReasons.map((reason) => <li key={reason}>{reason}</li>)}</ul></div>
