  ...
  const requestQuote = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const configuration = [
        ...
        `Appgebruikers: ${appUsers} × ${euro.format(ALARM_PRICES.appYearLicense)} per jaar`, `Dedicated devices: ${devices} × ${euro.format(ALARM_PRICES.devicePurchase)} + ${euro.format(ALARM_PRICES.deviceYearLicense)} per jaar`, `Swissphone C35-pagers: ${pagers} × ${euro.format(ALARM_PRICES.pagerWithCharger)} incl. oplaadstation`,
    ...
    <main className="alarm-configurator">
      ...
      <section className="configurator-shell" aria-label="Alarmeringsconfigurator">
        <div className="questions-panel" id="alarm-vragen">
          ...
          <Question number="04" title="Wie ontvangen de meldingen?" subtitle="Combineer apps, dedicated devices en RF-pagers naar behoefte.">
            <div className="receiver-grid"><Receiver title="App op eigen telefoon" subtitle={`${euro.format(ALARM_PRICES.appYearLicense)} per gebruiker per jaar`} value={appUsers} setValue={setAppUsers} /><Receiver title="Dedicated device" subtitle={`${euro.format(ALARM_PRICES.devicePurchase)} aanschaf + ${euro.format(ALARM_PRICES.deviceYearLicense)} per jaar`} value={devices} setValue={setDevices} /><Receiver title="Swissphone C35-pager" subtitle={`${euro.format(ALARM_PRICES.pagerWithCharger)} incl. oplaadstation`} value={pagers} setValue={setPagers} image="./swissphone-c35.png" /></div>
            ...
            {calculation.pagersOnQuote && <div className="inline-alert">Voor een ATEX-zone is een specifieke gecertificeerde pager nodig. De standaard C35-prijs is daarom niet in het totaal opgenomen.</div>}
          </Question>
